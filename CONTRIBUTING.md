# Contributing to helpthehomeless-lib

We're working hard to make *helpthehomeless-lib* the most powerful JavaScript library for working with Helpthehomeless. Our goal is to have *helpthehomeless-lib* be a library that can be used by anyone interested in Helpthehomeless, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://www.helpthehomeless.org/forum (Support Forum)
...
These are some global design goals in helpthehomeless-lib that any change must adhere.
...
When possible, helpthehomeless-lib objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of helpthehomeless-lib methods. For example, calling `JSON.stringify` on an helpthehomeless-lib object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'helpthehomeless' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the helpthehomeless object.

DO:
```javascript
var helpthehomeless = require('../');
var PublicKey = helpthehomeless.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add Altcoin-Cash git@github.com:Altcoin-Cash/helpthehomeless-lib.git
git pull --rebase Altcoin-Cash master
```
...
```
Finally go to [github.com/Altcoin-Cash/helpthehomeless-lib](https://github.com/Altcoin-Cash/helpthehomeless-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of helpthehomeless-lib.

If you have any questions feel free to post them to
[github.com/Altcoin-Cash/helpthehomeless-lib/issues](https://github.com/Altcoin-Cash/helpthehomeless-lib/issues).
